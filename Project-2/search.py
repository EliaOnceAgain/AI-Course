...
def tree_search(problem, frontier):
    ...
    frontier.append(Node(problem.initialize))
    while frontier:
        node = frontier.pop()
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                #print("#######>>>", list(map(lambda n: n.action, child.path()))[1:])
...
def depth_limited_search(problem, limit=50):
    ...
    return recursive_dls(Node(problem.initialize), problem, limit)
...
def bidirectional_search(problem):
    ...
    gF, gB = {problem.initialize : 0}, {problem.goal : 0}
    openF, openB = [problem.initialize], [problem.goal]
    closedF, closedB = [], []
    U = infinity
...
def hill_climbing(problem):
    ...
    current = Node(problem.initialize)
    while True:
...
def and_or_graph_search(problem):
    ...
    return or_search(problem.initialize, problem, [])
