...
class Overview():
    def __init__(self, grid_size):
        ...
        self.dictionary_device_spaceships_priorities = dict()
        self.dictionary_device_targets_priorities = dict()
    ...
    def get_possible_actions_updated(self):
        ...
        if not self.dictionary_device_spaceships_priorities[current_device]:
            if self.total_number_of_hits_per_device[current_device] == 0:
                print("Device Name: ",current_device)
                print("I am zeroed.")
                self.priority_calibration()
                current_device = self.devices[self.device_id_on_line]
        ...
        if not self.is_calibrated:
            is_done = False
            while not is_done:
                current_estimate = self.dictionary_device_spaceships_priorities[current_device][self.calibration_current_estimate_index]
                ...
                if current_estimate_spaceship_name not in self.undo_actions_dict:
                    if self.is_turned_on:
                        ...
                        if current_estimate[1] == 0:
                            ...
                            move_action = UtilsActions.get_move_action_priority_calibration(current_estimate,
                            ...
                            if move_action:
                                ...
                                if self.calibration_current_estimate_index < len(self.dictionary_device_spaceships_priorities[current_device]) - 1:
            ...
            print(self.dictionary_device_targets_priorities)
            ...
            if current_use_estimate[1] == 0:
                ...
                move_action = UtilsActions.get_move_action_priority_targets(current_use_estimate,
    ...
    def update_priority_calibration(self):
        ...
        if is_straight_line:
            ...
            if not found_occlusion:
                order_by = 0
                self.dictionary_device_spaceships_priorities[current_device].insert(0, [current_estimate_spaceship, order_by,
            ...
            order_by = sum(distances) - max(distances)
            distances_directions = sorted(zip(distances, directions), key=lambda dist: dist[0])
            self.dictionary_device_spaceships_priorities[current_device][self.calibration_current_estimate_index] = [current_estimate_spaceship, order_by,
    ...
    def priority_targets(self):
        ...
        current_estimate_spaceship = current_estimate[0]
        current_spaceship_coordinates = current_estimate_spaceship.coordinates
        all_distances_dict = self.get_all_distances_dict(specific_spaceship = current_estimate_spaceship)
    ...
    def update_priority_targets(self):
        ...
        if is_straight_line:
            ...
            order_by = sum(distances) - max(distances)
            distances_directions = sorted(zip(distances, directions), key=lambda dist: dist[0])
            self.dictionary_device_targets_priorities[current_device][self.calibration_current_estimate_index] = [
                current_target_coordinates, order_by,
                distances_directions]
    ...
    def update_action(self, action):
        ...
        if action[0] == 'move':
            ...
            current_device = self.devices[self.device_id_on_line]
            spaceship_active_name = self.dictionary_device_spaceships_priorities[current_device][self.calibration_current_estimate_index][0].name
            ...
            if self.total_number_of_hits_per_device[action_device] == 0:
                self.dictionary_device_spaceships_priorities[action_device] = []
                self.dictionary_device_targets_priorities[action_device] = []
            else:
                self.priority_targets()
...
class SpaceshipProblem(search.Problem):
    ...
    def actions(self, state):
        ...
        return state.get_possible_actions_updated()
