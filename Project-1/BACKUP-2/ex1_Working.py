...
class TargetPriorityManager(PriorityManager):
    ...
    def priority_target_update(self):
        ...
        if is_straight_line:
            ...
            del self.overview_obj.dictionary_device_targets_priorities[current_device][
                self.overview_obj.use_device_current_estimate_index]

            if not found_occlusion:
                priority_value = 0
                self.overview_obj.dictionary_device_targets_priorities[current_device].insert(0,
                                                                                              [
                                                                                                  current_target_coordinates,
                                                                                                  priority_value,
                                                                                                  (distances,
                ...
                self.add_priority_to_sorted_list_target(current_target_coordinates, current_device, priority_value,
        ...
        if current_use_estimate[1] == 0:
            ...
            for current_estimate_index in range(
                ...
                current_estimate = self.overview_obj.dictionary_device_targets_priorities[current_device][
                    current_estimate_index]
                current_estimate_actions_set = set()

                if current_estimate[1] == 0:
                    current_estimate_actions_set.add((
                    'use', current_estimate_spaceship_name, current_device, current_estimate[0]))
                else:
